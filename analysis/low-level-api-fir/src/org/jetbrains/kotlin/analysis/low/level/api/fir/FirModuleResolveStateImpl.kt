...
internal class FirModuleResolveStateImpl(
    ...
    override fun findSourceFirDeclaration(ktDeclaration: KtDeclaration): FirDeclaration {
        val firDeclaration = findSourceFirDeclarationByExpression(ktDeclaration)
        val originalDeclaration = ktDeclaration.originalDeclaration
        if (originalDeclaration != null) {
            firDeclaration.realFirAttr = findSourceFirDeclaration(originalDeclaration)
        }
...
private object RealFirKey : FirDeclarationDataKey()

private var <D : FirDeclaration> D.realFirAttr: D? by FirDeclarationDataRegistry.data(RealFirKey)
val <D : FirDeclaration> D.realFir: D? get() = realFirAttr
inline val <D : FirDeclaration> D.realFirOrSelf: D get() = realFir ?: this
